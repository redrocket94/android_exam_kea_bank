...
public class HRT_UpdateBill extends AsyncTask<Void, Void, CustomerData> {
    ...
    public HRT_UpdateBill(Long id, Long customerId, boolean isPaid) {
        this.id = id;
    ...
    protected CustomerData doInBackground(Void... params) {
        ...
        putBillData(userCustomerData);
        return null;
    }

    CustomerData getUserData() {
        try {
            String url = "http://10.0.2.2:8080/customers/" + customerId;
    ...
    void putBillData(CustomerData userCustomerData) {

        for (Bill bill :
                userCustomerData.getBills()) {
            if (bill.getId() == id) {
                bill.setPaid(isPaid);
            }
        }
        ...
        Customer updatedCustomer = new Customer(userCustomerData.getCustomerId(), userCustomerData.getFirstName(), userCustomerData.getLastName(), userCustomerData.getAge(), userCustomerData.getUsername(),
                userCustomerData.getPassword(), userCustomerData.getEmail(), Customer.Bank.valueOf(userCustomerData.getBank()), userCustomerData.getAccounts(), userCustomerData.getBills());

        RestTemplate restTemplate = new RestTemplate();
        restTemplate.getMessageConverters().add(new MappingJackson2HttpMessageConverter());
        restTemplate.getMessageConverters().add(new StringHttpMessageConverter());

        restTemplate.put("http://10.0.2.2:8080/customers/" + customerId, updatedCustomer);
    }
}